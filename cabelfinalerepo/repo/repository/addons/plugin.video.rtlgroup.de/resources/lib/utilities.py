...
class Transmission(object):
	...
	def retrieveContent(self, url, method='GET', REF=None, headers=None, cookies=None, allow_redirects=True, stream=None, data=None, json=None):
		send_token = True if method in ['GET', 'LOAD'] and addon.getSetting('verified_Account') == 'true' and self.auth_TOKEN != '0' else False
		ANSWER = None
		...
		try:
			if method in ['GET', 'LOAD']:
				result = self.session.get(url, headers=headers, allow_redirects=allow_redirects, verify=self.verify_ssl, stream=stream, timeout=30)
			elif method == 'POST':
			...
			ANSWER = result.json() if method in ['GET', 'POST'] else py2_enc(result.text)
